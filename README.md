...
Accessing the /login API endpoint with header variables ```id``` and ```otp``` will return a JWT token that shall be used in any further access to the API:
